...
function genePanel(geneName, maploc, rsid) {
  ...
  <text x="540" y="54"  text-anchor="middle" font-family="Afacad,sans-serif" font-size="11"  fill="#7a8fa6" letter-spacing="2">GENE VIEW</text>
  <text x="540" y="76"  text-anchor="middle" font-family="'Marcellus SC',Afacad,sans-serif"          font-size="16" fill="#e2e8f2" font-weight="700">${title}</text>

  <!-- Flanking grey bands -->
  <rect x="330" y="88"  width="420" height="24" rx="3" fill="#2a3d52" opacity="0.4"/>
  <text x="540" y="105" text-anchor="middle" font-family="Afacad,sans-serif" font-size="11" fill="#4a6680">${esc(above)}</text>
  <rect x="330" y="428" width="420" height="24" rx="3" fill="#2a3d52" opacity="0.4"/>
  <text x="540" y="445" text-anchor="middle" font-family="Afacad,sans-serif" font-size="11" fill="#4a6680">${esc(below)}</text>
  ...
  ${rsLabel ? `<text x="541" y="326" text-anchor="middle" font-family="Afacad,sans-serif" font-size="11" fill="#e84580" font-weight="600">${rsLabel}</text>` : ""}

  <!-- Density labels -->
  <text x="335" y="464" font-family="Afacad,sans-serif" font-size="11" fill="#3d5068">— ~70,000 base pairs in the gene</text>
  <text x="335" y="480" font-family="Afacad,sans-serif" font-size="11" fill="#3d5068">— ~14,000 uncommon variants · &lt;1% of humans have them</text>
  <text x="335" y="496" font-family="Afacad,sans-serif" font-size="11" fill="#34d399">— ~1,000 common variants · &gt;1% carry the alternate allele</text>`;
}
...
function allelePanel(rsid, refA, altA, maploc, geneName, proteinChange) {
  ...
  <text x="944" y="${y+20}" text-anchor="middle" font-family="Afacad,sans-serif" font-size="10" fill="#e84580">${esc(label)}</text>
  <rect x="864" y="${y+36}" width="160" height="12" rx="6" fill="#1f2d3d"/>
  <text x="868"  y="${y+46}" font-family="Afacad,sans-serif" font-size="10" fill="#3d5068">5&#x2032;</text>
  <text x="1017" y="${y+46}" font-family="Afacad,sans-serif" font-size="10" fill="#3d5068">3&#x2032;</text>
  <circle cx="944" cy="${y+42}" r="10" fill="${bg1}" stroke="${col1}" stroke-width="1.5"/>
  <text x="944" y="${y+46}" text-anchor="middle" font-family="Afacad,sans-serif" font-size="13" fill="${col1}" font-weight="700">${esc(letter1)}</text>
  <rect x="864" y="${y+54}" width="160" height="12" rx="6" fill="#1f2d3d"/>
  <text x="868"  y="${y+64}" font-family="Afacad,sans-serif" font-size="10" fill="#3d5068">5&#x2032;</text>
  <text x="1017" y="${y+64}" font-family="Afacad,sans-serif" font-size="10" fill="#3d5068">3&#x2032;</text>
  <circle cx="944" cy="${y+60}" r="10" fill="${bg2}" stroke="${col2}" stroke-width="1.5"/>
  <text x="944" y="${y+64}" text-anchor="middle" font-family="Afacad,sans-serif" font-size="13" fill="${col2}" font-weight="700">${esc(letter2)}</text>`;
  }

  return `
  <!-- ── PANEL 3: Allele state ── -->
  <rect x="810" y="20" width="270" height="520" rx="10" fill="#131820" stroke="#1f2d3d" stroke-width="1"/>
  <text x="945" y="54"  text-anchor="middle" font-family="Afacad,sans-serif" font-size="11"  fill="#7a8fa6" letter-spacing="2">ALLELE STATE</text>
  <text x="945" y="76"  text-anchor="middle" font-family="Afacad,sans-serif"          font-size="16" fill="#e2e8f2" font-weight="700">${esc(rsid || "")}</text>
  ${proteinChange ? `<text x="945" y="92"  text-anchor="middle" font-family="Afacad,sans-serif" font-size="11"  fill="#7a8fa6">${esc(proteinChange)}</text>` : ""}
  <text x="945" y="${proteinChange ? 108 : 94}"  text-anchor="middle" font-family="Afacad,sans-serif" font-size="11"  fill="#3d5068">${subtitle}</text>
  ...
  <text x="844"  y="456" text-anchor="middle" font-family="Afacad,sans-serif" font-size="11" fill="#34d399" font-weight="700">${esc(ref)}</text>
  <text x="858"  y="456" font-family="Afacad,sans-serif" font-size="11" fill="#7a8fa6">${esc(refName)} — reference allele</text>
  <circle cx="844" cy="474" r="8" fill="#12080f" stroke="#e84580" stroke-width="1.5"/>
  <text x="844"  y="478" text-anchor="middle" font-family="Afacad,sans-serif" font-size="11" fill="#e84580" font-weight="700">${esc(alt)}</text>
  <text x="858"  y="478" font-family="Afacad,sans-serif" font-size="11" fill="#7a8fa6">${esc(altName)} — variant allele</text>`;
}
